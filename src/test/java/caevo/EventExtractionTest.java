...
/**
 * Run the event classifier on some raw text and make sure the expected number
 * of events come out.
 */
public class EventExtractionTest extends TestCase {
  ...
  public void testRawToEvents() throws Exception {
    ...
    assertEquals("Number of events in 1st sentence.", 7,
        doc.getSentences().get(0).events().size());
    assertEquals("Number of events in 2nd sentence.", 2,
        doc.getSentences().get(1).events().size());
    // Some tense/aspect attributes.
    assertEquals("Event Tense check", TextEvent.Tense.PAST,
        doc.getSentences().get(0).events().get(0).getTense());
    assertEquals("Event Class check", TextEvent.Class.OCCURRENCE,
        doc.getSentences().get(0).events().get(0).getTheClass());
    assertEquals("Event Aspect check", TextEvent.Aspect.NONE,
        doc.getSentences().get(0).events().get(0).getAspect());
  }
