...
public class SieveStats {
  ...
  public void addProposedCount(int num) {
    numProposed += num;
  }

  public void addRemovedCount(int num) {
    numRemoved += num;
  }

  public void addClosureCount(int num) {
    numFromClosure += num;
  }
  ...
  public void addIncorrect(TLink link, TLink goldLink) {
    ...
    guessCounts
        .incrementCount(goldLink.getRelation() + " " + link.getRelation());
  }
  ...
  public void printOneLineStats() {
    ...
    if (sieveName.length() < 16)
      System.out.print("\t");
    if (sieveName.length() < 8)
      System.out.print("\t");
    // Overall precision.
    ...
    System.out.printf("p = %.2f\t(%d of %d)\n", precision,
        (int) correctLinks.size(), (int) totalGuessed);
  }
  ...
  public void printStats(PrintStream printer) {
    ...
    double unclosedP = (totalUnclosed == 0.0 ? 0.0
        : unclosedCorrect / totalUnclosed);
    printer.printf("\tnon-closed:\t%.2f\t(%d of %d)\n", unclosedP,
        (int) unclosedCorrect, (int) totalUnclosed);
    printer.printf("\tclosed:\t\t%.2f\t(%d of %d)\n", closedP,
        (int) closedCorrect, (int) totalClosed);
  ...
  /**
   * Create a file with the statistics for this sieve including all guessed
   * links.
   */
  public void dumpStatsToFile() {
    try {
      ...
      PrintStream writer = new PrintStream(new File(
          statsOutputDir + File.separator + this.sieveName + ".stats"));
      ...
      for (TLink link : correctLinks) {
        ...
        if (eiid1.startsWith("t"))
          writer.print(eiid1);
        else
          writer.print(link.getDocument().getEventByEiid(link.getId1()));
        writer.print("\t");
        if (eiid2.startsWith("t"))
          writer.print(eiid2);
        else
          writer.print(link.getDocument().getEventByEiid(link.getId2()) + "\t="
              + link.getRelation());
        writer.print("\n");
      }
      ...
      for (TLink link : incorrectLinks)
        writer.print(link.getDocument().getDocname() + "\t"
            + link.getDocument().getEventByEiid(link.getId1()) + "\t"
            + link.getDocument().getEventByEiid(link.getId2()) + "\t="
            + link.getRelation() + "\n");
      // writer.print(link.getDocument().getDocname() + "\t" + link + "\t
      // (orig=" + link.getOriginalRelation() + ")\n");
