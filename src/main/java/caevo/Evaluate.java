...
public class Evaluate {
  public static final TLink.Type[] labels = { TLink.Type.BEFORE,
      TLink.Type.AFTER, TLink.Type.SIMULTANEOUS, TLink.Type.INCLUDES,
      TLink.Type.IS_INCLUDED, TLink.Type.VAGUE };

  public static final String[] devDocs = { "APW19980227.0487.tml",
      "CNN19980223.1130.0960.tml", "NYT19980212.0019.tml",
      "PRI19980216.2000.0170.tml", "ed980111.1130.0089.tml" };

  public static final String[] testDocs = { "APW19980227.0489.tml",
  ...
  /**
   * Determines if the given TLink exists in the goldLinks, and its relation is
   * equal to or compatible (invertible or a more general relation) with the
   * gold link.
   * 
   * @param guessed
   *          A single TLink between two events.
   * @param goldLinks
   *          A list of gold tlinks.
  ...
  public static SieveDocuments getTrainSet(SieveDocuments docs) {
    ...
    for (SieveDocument doc : docs.getDocuments())
      if (!exists(doc.getDocname(), devDocs)
          && !exists(doc.getDocname(), testDocs))
  ...
  /**
   * This function makes sure that the two events in each TLink follow document
   * order. If not, then we invert the order so event 1 is 2 and 2 is 1, and we
   * also invert the ordering relation.
   * 
   * @param docs
   *          The documents to normalize.
   */
  public static void normalizeAllTlinksByTextOrder(SieveDocuments docs) {
    for (SieveDocument doc : docs.getDocuments()) {
      ...
      for (TLink link : doc.getTlinks()) {
        // System.out.println("normalizing " + link + " instance=" +
        ...
        if (link instanceof EventEventLink) {
          ...
          if (first == null || second == null)
            System.out.println("WARNING: document " + doc.getDocname()
                + " has a link with a null event: " + link);
          else if (!first.isBeforeInText(second)) {
            removal.add(link);
            TLink.Type invertedRelation = TLink
                .invertRelation(link.getRelation());
        ...
        // Time-Time links.
          ...
          if (first == null || second == null)
            System.out.println("WARNING: document " + doc.getDocname()
                + " has a link with a null timex: " + link);
          else if (first
              .getDocumentFunction() == Timex.DocumentFunction.CREATION_TIME
              && second
                  .getDocumentFunction() != Timex.DocumentFunction.CREATION_TIME) {
            flip = true;
          } else if (first
        ...
        // Event-Time links.
          ...
          if (link.getId1().startsWith("e")) {
            ...
            if (event == null || time == null)
              System.out.println("WARNING: document " + doc.getDocname()
                  + " has a link with a null event or timex: " + link);

            else if (!time.isDCT() && (time.getSid() < event.getSid()
                || (time.getSid() == event.getSid()
                    && time.getTokenOffset() < event.getIndex())))
              flip = true;
          }
          // time - event
          else {
            ...
            if (event == null || time == null)
              ...
              flip = true;
                ...
                || (event.getSid() == time.getSid()
                    && event.getIndex() < time.getTokenOffset()))
              flip = true;
          }
          ...
          System.out.println(
              "WARNING: a sieve is producing generic TLink instances. These must be specific! Evaluation is now unreliable.");
        }
      }
  ...
  private static String determineLinkType(TLink link) {
    ...
    /*
     * if( link.getId1().startsWith("e") && link.getId2().startsWith("e") ) {
     * if( link.getDocument().getEventByEiid(link.getId1()).getSid() ==
     * link.getDocument().getEventByEiid(link.getId2()).getSid() ) return
     * "InterSentEELink"; else return "IntraSentEELink"; } if(
     * (link.getId1().startsWith("e") && link.getId2().startsWith("t")) ||
     * (link.getId1().startsWith("t") && link.getId2().startsWith("e")) ) { if(
     * link.getDocument().getEventByEiid(link.getId1()).getSid() ==
     * link.getDocument().getEventByEiid(link.getId2()).getSid() ) return
     * "InterSentEELink"; else return "IntraSentEELink"; }
     */
  }

  /**
   * Full evaluation of guesses to gold links. This penalizes guesses for not
   * labeling everything. The goldDocs and guessedDocs should cover the same
   * docs.
   * 
   * @param goldDocs
   *          Gold tlinks in every document.
   * @param guessedDocs
   *          The guessed tlinks in every document.
   * @param sieveNames
   *          The list of sieve names, in order that they were applied to each
   *          document.
   * @param sieveStats
   *          A map from sieve names to their SieveStats objects.
   */
  public static void evaluate(SieveDocuments goldDocs,
      SieveDocuments guessedDocs, String[] sieveNames,
      Map<String, SieveStats> sieveStats) {
    ...
    for (SieveDocument guessedDoc : guessedDocs.getDocuments()) {
      ...
      // System.out.println("\t-> " + guessedDoc.getTlinks().size() + " guessed
      // links with " + goldDoc.getTlinks().size() + " gold links.");
      ...
      // System.out.println("EVALUATE: proposed links in " +
      // guessedDoc.getDocname() + " = " + proposed);
      ...
      for (TLink pp : proposed) {
        ...
        if (goldLink != null) {
          guessCounts
              .incrementCount(goldLink.getRelation() + " " + pp.getRelation());
        ...
        if (Evaluate.isLinkCorrect(pp, goldLinks)) {
          numCorrect++;
          breakdownNumCorrect
              .incrementCount(pp.isFromClosure() ? "closed" : "notclosed");
          ...
          if (pp.getOrigin() != null) {
            ...
            System.out.println("EVALUATE: unknown link origin: " + pp);
        }
        ...
        // wrong)
          ...
          if (pp.getOrigin() != null)
            sieveStats.get(pp.getOrigin()).addIncorrect(pp, goldLink);
          else
            System.out.println("EVALUATE: unknown link origin: " + pp);
        }
        // No gold link. We don't penalize for guessed links that aren't in
        // gold.
        else {
      ...
      // Check for gold links that were not predicted. Penalize for them being
      // missed.
      for (TLink gold : goldLinks) {
        if (!seenGoldLinks.contains(gold.getId1() + "," + gold.getId2())) {
          ...
          // System.out.println("Unlabeled gold: " + guessedDoc.getDocname() + "
          // " + gold);
        }
      }
    }
    ...
    double precision = (totalGuessed > 0 ? (double) numCorrect / totalGuessed
        : 0.0);
    ...
    int totalGoldNonVague = numCorrectNonVague + numIncorrectNonVague
        + numMissedNonVague;
    double precisionNonVague = (totalGuessedNonVague > 0
        ? (double) numCorrectNonVague / (double) totalGuessedNonVague : 0.0);
    double recallNonVague = (totalGuessedNonVague > 0
        ? (double) numCorrectNonVague / (double) totalGoldNonVague : 0.0);
    double f1NonVague = (precisionNonVague + recallNonVague > 0
        ? 2.0 * precisionNonVague * recallNonVague
            / (precisionNonVague + recallNonVague)
        : 0.0);
    ...
    System.out.printf(
        "precision\t= %.3f\t %d of %d\n" + "recall   \t= %.3f\t %d of %d\n"
            ...
            + "F1 (non-VAGUE)       \t= %.3f\n",
        precision, numCorrect, totalGuessed, recall, numCorrect, totalGold, f1,
        precisionNonVague, numCorrectNonVague, totalGuessedNonVague,
        recallNonVague, numCorrectNonVague, totalGoldNonVague, f1NonVague);
    System.out.println();

    System.out.printf(
        "Links directly from sieves: %.0f correct\t%.0f incorrect\t P=%.3f\n",
        breakdownNumCorrect.getCount("notclosed"),
        breakdownNumIncorrect.getCount("notclosed"),
        breakdownNumCorrect.getCount("notclosed")
            / (breakdownNumCorrect.getCount("notclosed")
                + breakdownNumIncorrect.getCount("notclosed")));
    System.out.printf(
        "Links from transitivity:    %.0f correct\t%.0f incorrect\t P=%.3f\n",
        breakdownNumCorrect.getCount("closed"),
        breakdownNumIncorrect.getCount("closed"),
        breakdownNumCorrect.getCount("closed")
            / (breakdownNumCorrect.getCount("closed")
                + breakdownNumIncorrect.getCount("closed")));
    ...
    for (String key : breakdownNumCorrect.keySet())
      System.out.printf(
          "Links from %s:\ttotal=%.0f\tP=%.0f/%.0f = %.3f\tR=%.0f/%d = %.3f\n",
          key,
          breakdownNumCorrect.getCount(key)
              + breakdownNumIncorrect.getCount(key),
          breakdownNumCorrect.getCount(key),
          breakdownNumCorrect.getCount(key)
              + breakdownNumIncorrect.getCount(key),
          breakdownNumCorrect.getCount(key) / (breakdownNumCorrect.getCount(key)
              + breakdownNumIncorrect.getCount(key)),
          breakdownNumCorrect.getCount(key), totalGold,
          breakdownNumCorrect.getCount(key) / totalGold);
  ...
  /**
   * Find the label with the most counts, and print the baseline if you always
   * guessed that one.
   */
  ...
  public static void printDatasetStats(Counter<TLink.Type> labelCounts,
      PrintStream printer) {
    printer.println(
        "GOLD LABEL COUNTS (out of " + (int) labelCounts.totalCount() + ")");
    for (TLink.Type label : labels) {
      int numtabs = (label.toString().length() > 7 ? 1 : 2);
      printer.print(label + "\t");
      if (numtabs == 2)
        printer.print("\t");
      printer.printf("%d\t%.0f%%\n", (int) labelCounts.getCount(label),
          (100.0 * labelCounts.getCount(label) / labelCounts.totalCount()));
    }
  }

  /**
   * Print the confusion matrix for the 6 label types. Each String key should be
   * a pair separated by a single space, representing a guess for a gold label:
   * e.g., "before after"
   * 
   * @param guessCounts
   *          Number of times each guessed label was made for each gold label.
   */
  ...
  public static void printConfusionMatrix(Counter<String> guessCounts,
    ...
    for (TLink.Type label2 : labels)
      printer.print("\t" + label2.toString().substring(0,
          Math.min(label2.toString().length(), 6)));
    printer.println("\t(guesses)");

    for (TLink.Type label1 : labels) {
      printer.print(label1.toString().substring(0,
          Math.min(label1.toString().length(), 6)) + "\t");

      for (TLink.Type label2 : labels) {
        if (guessCounts.containsKey(label1 + " " + label2))
          printer
              .print((int) guessCounts.getCount(label1 + " " + label2) + "\t");
        else
          printer.print("0\t");
      }
      printer.println();
   ...
   *          Number of times each guessed label was made for each gold label.
   */
  public static void printPerRelationPRF(Counter<String> guessCounts,
      Counter<TLink.Type> goldLabelCounts) {
    printPerRelationPRF(guessCounts, goldLabelCounts, System.out);
  }

  public static void printPerRelationPRF(Counter<String> guessCounts,
      Counter<TLink.Type> goldLabelCounts, PrintStream printer) {
    printer.println("-------------------------------\nPer Relation P/R/F1");
    for (TLink.Type label1 : labels) {
      printer.print(label1.toString().substring(0,
          Math.min(label1.toString().length(), 6)) + "\t");
      ...
      printer.println(String.format(
          "p=%.2f\tr=%.2f\tf1=%.2f\tcorrect=%.0f\tguessed=%.0f\tpossible=%.0f",
          precision, recall, f1, correct, totalGuessed, totalPossible));
    }
  }
