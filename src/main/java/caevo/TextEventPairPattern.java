...
/**
 * Represents a set of TextEvents pairs with certain fixed values for some
 * attributes (e.g. tense, aspect, and class). For example, one possible pattern
 * is (((Tense=PAST, Aspect=PROGRESSIVE),(Tense=FUTURE, Aspect=NONE)),
 * SAME_SENTENCE) representing all pairs of TextEvents in which the first is in
 * the past progressive, the second is in the future, and they are both in the
 * same sentence.
 * 
...
public class TextEventPairPattern implements Serializable {
  ...
  public enum SentenceRelation {
    SAME_SENTENCE, DIFFERENT_SENTENCE
  }
  ...
  public TextEventPairPattern(TextEvent canonicalEvent1,
      TextEvent canonicalEvent2, boolean keepClass, boolean keepTense,
      boolean keepAspect, boolean keepSentenceRelation) {
    this.p1 = new TextEventPattern();
    this.p2 = new TextEventPattern();
    setFromCanonicalEvent(canonicalEvent1, canonicalEvent2, keepClass,
        keepTense, keepAspect, keepSentenceRelation);
  }

  public void setFromCanonicalEvent(TextEvent canonicalEvent1,
      TextEvent canonicalEvent2, boolean keepClass, boolean keepTense,
      boolean keepAspect, boolean keepSentenceRelation) {
    this.p1.setFromCanonicalEvent(canonicalEvent1, keepClass, keepTense,
        keepAspect);
    this.p2.setFromCanonicalEvent(canonicalEvent2, keepClass, keepTense,
        keepAspect);
  ...
  public boolean matches(TextEvent event1, TextEvent event2) {
    return this.p1.matches(event1) && this.p2.matches(event2)
        && (this.sentenceRelation == null
            || (this.sentenceRelation == SentenceRelation.SAME_SENTENCE
                && event1.getSid() == event2.getSid())
            || (this.sentenceRelation == SentenceRelation.DIFFERENT_SENTENCE
                && event1.getSid() != event2.getSid()));
  }
  ...
  public int hashCode() {
    return this.p1.hashCode() ^ this.p2.hashCode()
        ^ ((this.sentenceRelation == null) ? 0
            : this.sentenceRelation.toString().hashCode());
  }

  public boolean equals(Object o) {
    TextEventPairPattern p = (TextEventPairPattern) o;
    return p.p1.equals(this.p1) && p.p2.equals(this.p2)
        && this.sentenceRelation == p.sentenceRelation;
  }
