...
/**
 * EventCorefSieve labels pairs of coreferent event mentions as simultaneous.
 * The coreference is determined by outside tools, added through the
 * SieveDocument event coref sets.
 * 
    ...
    for (List<TextEvent> corefEvents : corefEventSets) {
      for (int i = 0; i < corefEvents.size(); i++) {
        for (int j = i + 1; j < corefEvents.size(); j++) {
          proposed.add(new EventEventLink(corefEvents.get(i).getEiid(),
              corefEvents.get(j).getEiid(), TLink.Type.SIMULTANEOUS));
        }
      }
    }
