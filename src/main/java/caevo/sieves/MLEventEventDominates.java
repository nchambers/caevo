...
/**
 * Machine learned event-event pairs that are in a syntactic domination
 * relationship.
 * 
 ...
 * 
 * MLEventEventDominates p=0.47 60 of 128 Non-VAGUE: p=0.54 60 of 112 (Baseline
 * 0.43)
 * 
 * @author chambers
 */
public class MLEventEventDominates extends MLEventEventSameSent {
  Classifier<String, String> eeSameSentDominatesClassifier = null; // intra-sentence,
                                                                   // event-event
                                                                   // syntactically
                                                                   // dominates
  ...
  private void readClassifiers() {
    ...
    eeSameSentDominatesClassifier = Util
        .readClassifierFromFile(this.getClass().getResource(path));
    if (eeSameSentDominatesClassifier == null)
      System.out.println(
          "ERROR: MLEventEventDominates could not read its classifier at: "
              + path);
  }
  ...
  public void train(SieveDocuments docs) {
    ...
    eeSameSentDominatesClassifier = TLinkClassifier.train(data,
        featMinOccurrence);
