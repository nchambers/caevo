 ...
 *
 * MLEventTimeDiffSent p=0.37 36 of 97 Non-VAGUE: p=0.41 36 of 87 (Baseline
 * 0.35)
 *
 * @author chambers
 */
public class MLEventTimeDiffSent implements Sieve {
  Classifier<String, String> etDiffSentClassifier = null; // intra-sentence
                                                          // event-event links.
  Classifier<String, String> etDiffSentExistsClassifier = null; // binary, is
                                                                // there a link
                                                                // or not?
  ...
  private void init() {
    // Flags
    try {
      debug = CaevoProperties.getBoolean("MLEventTimeDiffSent.debug", false);
  ...
  public List<TLink> extractDiffSentenceEventTimeLinks(SieveDocument doc) {
        ...
        for (TextEvent event : events) {
          for (Timex timex : timexesNext) {
            ...
            Pair<String, Double> labelProb = TLinkClassifier
                .getLabelProb(etDiffSentClassifier, rvf);
    ...
    if (debug)
      System.out.println("Returning diff sentence e-t tlinks: " + tlinks);
    return tlinks;
  }
  ...
  public void train(SieveDocuments docs) {
    ...
    try {
      ...
      System.out.println(
          "ERROR: couldn't write classifiers to file in MLEventEventDiffSent");
      ex.printStackTrace();
    }
  }
