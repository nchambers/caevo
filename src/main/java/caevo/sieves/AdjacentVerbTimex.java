 ...
 * in which the two are "adjacent" in some sense, and the event is a verb. Two
 * notions of adjacency implemented thus far are based on surface distance and
 * relative position in a dependency parse graph.
 * 
 * 
 * Parameters These parameters are booleans. If true, classify the corresponding
 * pairs AdjacentVerbTimex.EVENT_BEFORE_TIMEX - classify a pair if the event
 * comes before the timex in the text AdjacentVerbTimex.TIMEX_BEFORE_EVENT
 * AdjacentVerbTimex.EVENT_GOVERNS_TIMEX - classidy a pair if the event governs
 * the timex AdjacentVerbTimex.TIMEX_GOVERNS_EVENT
 * 
 * This parameter specifies the maximum number of words that may intervene
 * between the verb and timex such that they are still considered "adjacent".
 * AdjacentVerbTimex.numInterWords
 * 
 * timex before event - true p=0.64 89 of 139 Non-VAGUE: p=0.75 89 of 119 timex
 * before event - false p=0.70 74 of 106 Non-VAGUE: p=0.86 74 of 86
 *
 * The biggest problem is when the system says IS_INCLUDED and the correct
 * answer is VAGUE. I attribute this to annotation difficulties; I think in many
 * such cases the system is correct. More analysis to come.
 * 
 * EVENT_BEFORE_TIMEX uses preposition-based rules (default is_included)
 * TIMEX_BEFORE_EVENT default is_included; vague for event "now"; vague when
 * "said" precedes timex TODO: generalize the third rule to reporting verbs
 * TODO: check if that reporting verb governs the event after the timex
 * EVENT_GOVERNS_TIMEX just returns is_included TODO: add more rules; this is
 * hard because many errors are with event = "now", or vague
 * 
 * @author cassidy
 */
public class AdjacentVerbTimex implements Sieve {
  ...
  public List<TLink> annotate(SieveDocument doc, List<TLink> currentTLinks) {
    ...
    try {
      EVENT_BEFORE_TIMEX = CaevoProperties
          .getBoolean("AdjacentVerbTimex.EVENT_BEFORE_TIMEX", true);
      TIMEX_BEFORE_EVENT = CaevoProperties
          .getBoolean("AdjacentVerbTimex.TIMEX_BEFORE_EVENT", true);
      EVENT_GOVERNS_TIMEX = CaevoProperties
          .getBoolean("AdjacentVerbTimex.EVENT_GOVERNS_TIMEX", true);
      TIMEX_GOVERNS_EVENT = CaevoProperties
          .getBoolean("AdjacentVerbTimex.TIMEX_GOVERNS_EVENT", true);
      numInterWords = CaevoProperties.getInt("AdjacentVerbTimex.numInterWords",
          0);
    } catch (IOException e) {
    ...
    for (SieveSentence sent : sentList) {
      ...
      Tree tree = null; // initialize to null so we don't end up loading it
                        // unless timexes are in the sentence

      if (debug == true) {
        System.out.println("DEBUG: adding tlinks from " + doc.getDocname()
      ...
      for (Timex timex : sent.timexes()) {
        ...
        for (TextEvent event : sent.events()) {
          ...
          if (!validateEvent(event, tree, sent))
            continue;
          ...
          boolean verbIsBeforeTimex = (eventToTimexDist <= (numInterWords + 1)
              && eventToTimexDist >= 1);
          boolean timexIsBeforeVerb = (eventToTimexDist
              * (-1) <= (numInterWords + 1) && eventToTimexDist <= -1);

          // Get parameters used for "structured" notion of adjacency
          // Dependency relation between event and time if applicable
          // Initialize booleans for dependency relation direction, updated
          // below
          ...
          // Check if event governs timex, and if so save the dependency
          // relation depRel
          eventTimeDep = getDepSentIndexPair(deps, event.getIndex(),
              timex.getTokenOffset());
          if (eventTimeDep != null) {
            depRel = eventTimeDep.reln();
            eventDoesGovernTimex = true;
          }
          // If not, check if timex governs event, and if so save the dependency
          // relation depRel
          else {
            eventTimeDep = getDepSentIndexPair(deps, timex.getTokenOffset(),
                event.getIndex());
            if (eventTimeDep != null) {
              depRel = eventTimeDep.reln();
          ...
          /*
           * At this point, if there's a dependency relationship between the
           * event and the time We know what it is (depRel) and the direction
           * (eventGovernsTimex vs timexGovernsEvent)
           */

          // Now, we determine what TLink (if any) to add to proposed for the
          // event/timex pair
          TLink tlink = null;
          ...
          if (EVENT_BEFORE_TIMEX && verbIsBeforeTimex) {
            flatTlink_et = eventBeforeTimex(eventToTimexDist, event, timex,
                sent, tree);
          }
          // if timex is before verb, use these rules...
          else if (TIMEX_BEFORE_EVENT && timexIsBeforeVerb) {
            if (eventDoesGovernTimex == false) {
              // If event governs a timex that comes before it, label it vague -
              // 8/13
              // probably can be fixed with lexical features
              // flatTlink_te = null;
              flatTlink_te = new EventTimeLink(event.getEiid(), timex.getTid(),
                  TLink.Type.VAGUE);
            } else {
              flatTlink_te = timexBeforeEvent(eventToTimexDist, event, timex,
                  sent, tree); // 8/11
              if (debug == true) {
                System.out.printf("E GOV T: %s(%s) %s(%s)\n%s",
                    event.getString(), event.getId(), timex.getText(),
                    timex.getTid(), sent.sentence());
              }
            }
          }
          ...
          // if verb governs timex, use these rules
            ...
            depTlink_et = eventGovernsTimex(eventToTimexDist, event, timex,
                sent, tree, depRel);
          ...
          // if timex governs verb, use these rules
            ...
            depTlink_te = timexGovernsEvent(eventToTimexDist, event, timex,
                sent, tree, depRel);
          }

          // heriarchy of preference is already determined in the above
          // conditional blocks
          // take flat unless the event and timex are not within the
          // numInterWords range,
          // in which case back off to dep. By setting numInterWords low we only
          // use the
          // flat method when the event and timex are very close together. This
          // seems
          // to work best.
          if (depTlink_te != null)
            tlink = depTlink_te;
   ...
   * @return a non-null tlink just in case the implemented criteria are
   *         satisfied
   */
  private TLink timexGovernsEvent(int eventToTimexDist, TextEvent event,
    ...
    TLink tlink = null;
    tlink = new EventTimeLink(event.getEiid(), timex.getTid(),
   ...
   *         satisfied
   */
  private TLink timexBeforeEvent(int eventToTimexDist, TextEvent event,
      Timex timex, SieveSentence sent, Tree tree) {
    TLink tlink = null;
    // if the timex is "now", return a vague tlink unless the event is in the
    // progressive tense,
    ...
    // otherwise, return is_included unless the word directly preceding the
    // timex is "said".
    // this is the naive (flat) way to cover a common case that looks like this:
    //
    else {
      if (timex.getTokenOffset() > 1
          && getTextAtIndex(timex.getTokenOffset() - 1, sent).equals("said")) {
        tlink = new EventTimeLink(event.getEiid(), timex.getTid(),
            TLink.Type.VAGUE);
      } else
    ...
    if (debug == true) {
      System.out.printf("TimexVerb: %s\n%s | %s", sent.sentence(),
          event.getString(), timex.getText());
    }
    return tlink;
  }

  private TLink eventBeforeTimex(int eventToTimexDist, TextEvent event,
      Timex timex, SieveSentence sent, Tree tree) {
    // If there are no intervening words, label is_included
    TLink tlink = null;

    if (eventToTimexDist == 1) {
      return new EventTimeLink(event.getEiid(), timex.getTid(),
          TLink.Type.IS_INCLUDED);
    } else {
   ...
   * @param sentIndex1
   * @param sentIndex2
   * @return null unless there is a dependency in which the item at sentIndex1
   *         governs the item at sentIndex2
   */
  private TypedDependency getDepSentIndexPair(List<TypedDependency> deps,
      int sentIndex1, int sentIndex2) {
   ...
   */
  private boolean validateEvent(TextEvent event, Tree tree,
      SieveSentence sent) {
    String eventPos = posTagFromTree(tree, sent, event.getIndex());
    if (!eventPos.startsWith("VB"))
      return false;
    else
      return true;
  }
  ...
  private String posTagFromTree(Tree tree, SieveSentence sent, int index) {
    // tree might be null; we keep it null until we need a pos for the first
    // time for a given sentence
    if (tree == null)
  ...
  private TLink applyPrepVerbTimexRules(TextEvent event, Timex timex,
      String prepText) {
  ...
  // public void checkTLink(TLink tlink, List<TLink> proposed) throws
  // IllegalStateException{
  // int numTLinks = proposed.size();
  // for (int t = 0; t < numTLinks; t++) {
  // if (tlink.coversSamePair(proposed.get(t))) {
  // throw new IllegalStateException("Cannot add a tlink between a pair of
  // events for which there is already a tlink in proposed");
  // }
  // }
  // }
