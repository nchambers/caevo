...
import caevo.SieveDocumentsAnalyzer;
...
/**
 * FrequencyVagueSieve looks at training data to find out if certain pairs of
 * tenses, classes, and aspects typically are related by VAGUE TLinks, and then
 * makes up rules based on these patterns. Rules are created for pairs of event
 * types that tend to have VAGUE links above a certain precision across a
 * minimum number of examples.
 * 
 * NOTE: MLVagueSieve should be used instead of this.
 * 
 * The following table contains the current results for the train and dev sets
 * when training on train.
 * 
 * p_min ex_min train-set dev-set .6 20 0.66 (932 of 1410) 0.50 (100 of 200) .7
 * 20 0.76 (358 of 472) 0.48 (31 of 65) .8 20 0.83 (186 of 225) 0.41 (14 of 34)
 * .6 80 0.65 (326 of 499) 0.51 (31 of 61) .7 80 0.72 (58 of 81) 1.00 (6 of 6)
 * .7 50 0.75 (233 of 311) 0.52 (26 of 50) .8 50 0.83 (99 of 120) 0.45 (10 of
 * 22)
 * 
...
public class FrequencyVagueSieve implements Sieve {
  ...
  /*
   * The following static integers and matrix encode which types of rules are
   ...
   * type of rule. Each type of rule matches a subset of the attributes for
   * events. For example, one rule type considers only event classes, whereas
   * another considers classes and tenses. An array A encodes a rule type
   * considering attribute X iff A[X_INDEX]=true.
   */
  ...
  public FrequencyVagueSieve() {
    ...
    try {
      this.ruleSavePath = CaevoProperties.getString(
          "FrequencyVagueSieve.ruleSavePath",
          "src/main/resources/models/tlinks/FrequencyVagueSieve");
      this.minRulePrecision = CaevoProperties
          .getDouble("FrequencyVagueSieve.minRulePrecision", .7);
      this.minRuleExamples = CaevoProperties
          .getInt("FrequencyVagueSieve.minRuleExamples", 50);

      ObjectInputStream ois = new ObjectInputStream(
          new FileInputStream(this.ruleSavePath));
      Object o = ois.readObject();
      ois.close();
      this.vaguePatterns = (HashSet<TextEventPairPattern>) o;
    } catch (Exception ex) {
      System.out.println("FrequencyVagueSieve: Had fatal trouble loading "
          + this.ruleSavePath);
      ex.printStackTrace();
    }
  }
  ...
  /*
   * Keep track of types of event pairs which are almost always vague, given at
   * least some number of examples.
   */
  public void train(SieveDocuments trainingInfo) {
    this.vaguePatterns = new HashSet<TextEventPairPattern>();
    SieveDocumentsAnalyzer analyzer = new SieveDocumentsAnalyzer(trainingInfo);
    for (int t = 0; t < FrequencyVagueSieve.distinguishedFeatures.length; t++) {
      HashMap<TextEventPairPattern, HashMap<TLink.Type, Integer>> linkTypeCounts = analyzer
          .getEventEventLinkCounts(
              FrequencyVagueSieve.distinguishedFeatures[t][CLASS_INDEX],
              FrequencyVagueSieve.distinguishedFeatures[t][TENSE_INDEX],
              FrequencyVagueSieve.distinguishedFeatures[t][ASPECT_INDEX],
              FrequencyVagueSieve.distinguishedFeatures[t][SENTENCE_INDEX]);

      for (Entry<TextEventPairPattern, HashMap<TLink.Type, Integer>> e1 : linkTypeCounts
          .entrySet()) {
        ...
        double precision = e1.getValue().get(TLink.Type.VAGUE)
            / (double) totalCount;
        if (precision >= this.minRulePrecision
            && totalCount >= this.minRuleExamples) {
          this.vaguePatterns.add(e1.getKey());
          System.out.println("FrequencyVagueSieve found rule: " + e1.getKey()
              + " Precision: " + precision + " Count: " + totalCount);
        }
