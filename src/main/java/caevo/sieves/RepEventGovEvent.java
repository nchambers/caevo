...
public class RepEventGovEvent implements Sieve {
  ...
  public List<TLink> annotate(SieveDocument doc, List<TLink> currentTLinks) {
    ...
    for (SieveSentence sent : doc.getSentences()) {
        ...
        if (govEvent != null && depEvent != null) {
          if (govEvent.getTheClass().equals(TextEvent.Class.REPORTING)) {
            ...
            if (relation != null) {
              ...
              // System.out.println("GOVDEP:\t\t" + new
              // EventEventLink(govEvent.getEiid(), depEvent.getEiid(),
              // relation));
            }
          }
        }
      ...
      // Find all event-event pairs where one syntactically dominates the other.
        ...
        for (TextEvent depEvent : dominatedEvents) {
          if (govEvent.getTheClass().equals(TextEvent.Class.REPORTING)) {
            ...
            if (relation != null) {
              proposed.add(new EventEventLink(govEvent.getEiid(),
                  depEvent.getEiid(), relation));
              // System.out.println("DOMINATED:\t" + new
              // EventEventLink(govEvent.getEiid(), depEvent.getEiid(),
              // relation));
            }
          }
   ...
   * 
   * @param t1
   *          Governing event's tense
   * @param a1
   *          Governing event's aspect
   * @param t2
   *          Dependent event's tense
   * @param a2
   *          Dependent event's aspect
