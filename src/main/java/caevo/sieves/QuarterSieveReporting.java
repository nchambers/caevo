 ...
 * quarter, appears directly after a reporting verb, it tends to be the case
 * that the expression is not a temporal argument but rather modifies one of the
 * verbs arguments. For example "company reported third quarter losses of $3
 * million...", "third quarter" modifies" losses, and "third quarter losses" is
 * itself an argument of "reported".
 * 
 * @author cassidy
 */
public class QuarterSieveReporting implements Sieve {
  ...
  private String textQuarterRegex = "(first|second|third|fourth|1st|2nd|3rd|4th)(\\s|-)quarter";
  ...
  // returns the label appropriate for the word between the verb and quarter
  // timex
  // could be improved by checking to see if the quarter is to occur in the
  // future,
  ...
  /**
   * validateTime ensures that timex value is of a certain form one option is:
   * YYYY-MM-DD or YYYY-MM or YYYY The idea is that some time expressions are
   * modifiers of arguments of the verb, and not arguments themselves. For
   * example, "X said Tuesday that ..." vs. "X said Tuesday's earnings were ..."
   * In the latter case, 'said' may not be included_in Tuesday. A very common
   * example is when the time expression is a quarter, since the verb is almost
   * always after the quarter.
   */
  ...
  private Boolean validateTimex(Timex timex) {
    ...
    if (matchText.matches() && matchVal.matches())
      return true;
    else
      return false;
  }
