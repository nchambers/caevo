...
/**
 * This sieve deals with event pairs, where an I_ACTION event governs another event.
 * 
 * @author chambers
 * @author bethard (code copied from)
 */
		...
		for (SieveSentence sent : doc.getSentences()) {
			...
			for (TypedDependency dep : deps) {
				...
				if( govEvent != null && depEvent != null ) {
					if( govEvent.getTheClass() == TextEvent.Class.I_STATE ) {
						Type relation = null;
						System.out.println("I_STATE: " + govEvent);
						System.out.println("\treln: " + dep);
						System.out.println("\tdep event: " + depEvent);
						
						// "think and consider" (3 of 3)
//						if( dep.reln().toString().startsWith("conj") )
//							relation = Type.VAGUE;
...
//						}
						...
						if( depEvent.getTheClass().equals(TextEvent.Class.I_STATE) ) {
