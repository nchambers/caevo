...
# ann1's frequency counts are in the rows, ann2's in the columns
...
sub readRelations {
    ...
    open(IN,$file);
    while( $line = <IN> ) {
	if( $line =~ /\s*([et]\d+)\s+([a-zA-Z]+)\s+(t0)\s*$/ ) {
	    my $pair = "$1 $3";
	    my $rel = $2;
	    $rels{$pair} = $rel;
	}
	elsif( $line =~ /\s*(t0)\s+([a-zA-Z]+)\s+([et]\d+)\s*$/ ) {
	    my $pair = "$1 $3";
	    my $rel = $2;
	    $rels{$pair} = $rel;
	}
	elsif( $line =~ /\s*([et]\d+)\s+(t0)\s+([a-zA-Z]+)\s*$/ ) {
	    my $pair = "$1 $2";
	    my $rel = $3;
	    $rels{$pair} = $rel;
	}
	elsif( $line =~ /\s*(t0)\s+([et]\d+)\s+([a-zA-Z]+)\s*$/ ) {
	    my $pair = "$1 $2";
	    my $rel = $3;
	    $rels{$pair} = $rel;
	}
	elsif( $line =~ /\s*([et]\d+)\s+([a-zA-Z]+)\s+([et]\d+)\s*$/ ) {}
